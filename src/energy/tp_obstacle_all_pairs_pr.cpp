...
namespace rsurfaces
{
    ...
    mreal TPObstacleAllPairs_Projectors::Energy(T1 alphahalf, T2 betahalf)
    {
    ...
    template<typename T1, typename T2>
    mreal TPObstacleAllPairs_Projectors::DEnergy(T1 alphahalf, T2 betahalf)
    {
...
//            #pragma omp simd aligned( B, Y1, Y2, Y3, Q11, Q12, Q13, Q22, Q23, Q33 : ALIGN ) reduction( + : i_sum)
    ...
    double TPObstacleAllPairs_Projectors::Value()
    {
    ...
    void TPObstacleAllPairs_Projectors::Differential(Eigen::MatrixXd &output)
    {
        bvh = bvhSharedFrom->GetBVH();
        if (!bvh)
        {
        ...
        if( bvh->near_dim != 10)
        {
            eprint("in TPObstacleAllPairs_Projectors::Differential: bvh->near_dim != 10");
        }
        if( o_bvh->near_dim != 10)
        {
            eprint("in TPObstacleAllPairs_Projectors::Differential: o_bvh->near_dim != 10");
        }
    ...
    void TPObstacleAllPairs_Projectors::Update()
    {
        // Nothing needs to be done
    }

    // Get the mesh associated with this energy.
    MeshPtr TPObstacleAllPairs_Projectors::GetMesh()
    {
        return mesh;
    }

    // Get the geometry associated with this geometry.
    GeomPtr TPObstacleAllPairs_Projectors::GetGeom()
    {
        return geom;
    }

    // Get the exponents of this energy; only applies to tangent-point energies.
    Vector2 TPObstacleAllPairs_Projectors::GetExponents()
    {
    ...
    OptimizedClusterTree *TPObstacleAllPairs_Projectors::GetBVH()
    {
        return 0;
    }
    ...
    double TPObstacleAllPairs_Projectors::GetTheta()
    {
        return 0.;
    }
