...
namespace rsurfaces
{
    namespace Hs
    {
        void ProjectViaSchur(const HsMetric &hs, Eigen::MatrixXd &gradient, Eigen::MatrixXd &dest)
        {
            ...
            Eigen::VectorXd temp;
            temp.setZero(hs.Schur().C.cols());
            MatrixUtils::MatrixIntoColumn(gradient, temp);
            
            ProjectViaSchurV(hs, temp, temp);
        ...
        {
            size_t nRows = hs.Schur().M_A.rows();
            int nIters = 0;
            ...
            while (nIters < newtonSteps)
            {
                ...
                MatrixUtils::SolveDenseSystem(hs.Schur().M_A, vals, vals);
                // Apply C^T
                Eigen::VectorXd correction = hs.Schur().C.transpose() * vals;
                // Apply A^{-1}
