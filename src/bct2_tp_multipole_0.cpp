...
//######################################################################################################################################
//      Multipole Energy -- Integer Exponents
//######################################################################################################################################

mreal BlockClusterTree2::FarFieldEnergyInteger0()
{
    mreal real_minus_betahalf = -beta/2.;
    
    mint int_minus_betahalf = std::round(real_minus_betahalf);
...
//        #pragma omp simd aligned( Y1, Y2, Y3, M1, M2, M3 : ALIGN )
        for( mint k = b_outer[i]; k < b_outer[i+1]; ++k )
        {
            ...
            if( i <= j )
            {
                ...
                block_sum += ( intpow( fabs(rCosPhi), int_alpha ) + intpow( fabs(rCosPsi), int_alpha) ) * intpow( r2, int_minus_betahalf ) * B[j];
            }
        }
    ...
    return sum;
}; //FarFieldEnergyInteger0


mreal BlockClusterTree2::DFarFieldEnergyInteger0Helper()
{
    
    mreal real_alpha_minus_2 = alpha - 2;
    mint  int_alpha_minus_2  = std::round( real_alpha_minus_2 );
    
    mreal real_minus_betahalf_minus_1 = -beta/2. - 1.;
    mint  int_minus_betahalf_minus_1  = std::round( real_minus_betahalf_minus_1) ;
...
//        #pragma omp simd aligned( Y1, Y2, Y3, M1, M2, M3 : ALIGN ) reduction( + : block_sum )
        for( mint k = b_outer[i]; k < b_outer[i+1]; ++k )
        {
            ...
            if( i<= j )
            {
                ...
                mreal rBetaMinus2 = intpow( r2, int_minus_betahalf_minus_1 );
                mreal rBeta = rBetaMinus2 * r2;
    
                mreal rCosPhiAlphaMinus1 = intpow( fabs(rCosPhi), int_alpha_minus_2 ) * rCosPhi;
                mreal rCosPhiAlpha = rCosPhiAlphaMinus1 * rCosPhi;
                
                mreal rCosPsiAlphaMinus1 = intpow( fabs(rCosPsi), int_alpha_minus_2 ) * rCosPsi;
                mreal rCosPsiAlpha = rCosPsiAlphaMinus1 * rCosPsi;
    ...
    return sum;
}; //DFarFieldEnergyInteger0Helper


mreal BlockClusterTree2::NearFieldEnergyInteger0()
{
    // Caution: This functions assumes that S = T!!!
...
//                    #pragma omp simd aligned( Y1, Y2, Y3, M1, M2, M3 : ALIGN ) reduction( + : block_sum )
                    for( mint j = ( b_i != b_j ? j_begin : i + 1 ); j < j_end; ++j ) // if i == j, we loop only over the upper triangular block, diagonal excluded
                    {
                        ...
                        mreal en = ( intpow( fabs(rCosPhi), int_alpha ) + intpow( fabs(rCosPsi), int_alpha) ) * intpow( r2, int_minus_betahalf );
    ...
    return sum;
}; //NearFieldEnergyInteger0


mreal BlockClusterTree2::DNearFieldEnergyInteger0Helper()
{
    // Caution: This functions assumes that S = T!!!
...
//                    #pragma omp simd aligned( B, Y1, Y2, Y3, M1, M2, M3 : ALIGN )
    ...
    return sum;
}; //DNearFieldEnergyInteger0Helper
