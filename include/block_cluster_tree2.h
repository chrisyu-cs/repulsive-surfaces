...
#include "block_cluster_tree.h" // only for definition of BCTKernelType
...
public:
    ...
    void Multiply( Eigen::VectorXd &input, Eigen::VectorXd &output, const mint k, BCTKernelType type, bool addToResult = false );     // <--- Main interface routine for Chris.
    
    void Multiply( Eigen::VectorXd &input, Eigen::VectorXd &output, BCTKernelType type, bool addToResult = false );     // <--- Main interface routine for Chris.
    
    void Multiply( Eigen::MatrixXd &input, Eigen::MatrixXd &output, BCTKernelType type, bool addToResult = false );     // <--- Main interface routine for Chris.
...
//    mreal fr_exponent;
    ...
    A_Vector<mreal> hi_diag;
    A_Vector<mreal> lo_diag;
    A_Vector<mreal> fr_diag;
...
//private:  // made public only for debugging
    ...
    void InternalMultiply( BCTKernelType type );
...
{
    mint b_m = far->b_m;
    mint  const * const restrict b_outer   = &far->b_outer[0];     mint  const * const restrict b_inner   = &far->b_inner[0];

    // Dunno why "restrict" helps with P_data. It is actually a lie here when S = T.
    mreal const * const restrict A  = &S->C_data[0][0];
    mreal const * const restrict X1 = &S->C_data[1][0];             mreal const * const restrict N1 = &S->C_data[4][0];
    mreal const * const restrict X2 = &S->C_data[2][0];             mreal const * const restrict N2 = &S->C_data[5][0];
    mreal const * const restrict X3 = &S->C_data[3][0];             mreal const * const restrict N3 = &S->C_data[6][0];

    mreal const * const restrict B  = &T->C_data[0][0];
    mreal const * const restrict Y1 = &T->C_data[1][0];             mreal const * const restrict M1 = &T->C_data[4][0];
    mreal const * const restrict Y2 = &T->C_data[2][0];             mreal const * const restrict M2 = &T->C_data[5][0];
    mreal const * const restrict Y3 = &T->C_data[3][0];             mreal const * const restrict M3 = &T->C_data[6][0];
    
    mreal sum = 0.;
    #pragma omp parallel for num_threads( thread_count ) schedule (guided, 8 ) reduction( + : sum)
    for( mint i = 0; i < b_m; ++i )
    {
        mreal x1 = X1[i];   mreal n1 = N1[i];
        mreal x2 = X2[i];   mreal n2 = N2[i];
        mreal x3 = X3[i];   mreal n3 = N3[i];
        ...
        for( mint k = b_outer[i]; k < b_outer[i+1]; ++k )
        {
            ...
            if( i <= j )
            {
                mreal v1 = Y1[j] - x1;  mreal m1 = M1[j];
                mreal v2 = Y2[j] - x2;  mreal m2 = M2[j];
                mreal v3 = Y3[j] - x3;  mreal m3 = M3[j];
...
mreal BlockClusterTree2::NearFieldEnergy()
{
    ...
    mint  const * const restrict b_row_ptr = &near->b_row_ptr[0];   mint  const * const restrict b_col_ptr = &near->b_col_ptr[0];
    mint  const * const restrict b_outer   = &near->b_outer[0];     mint  const * const restrict b_inner   = &near->b_inner[0];

    // Dunno why "restrict" helps with P_data. It is actually a lie here.
    mreal const * const restrict A  = &S->P_data[0][0];
    mreal const * const restrict X1 = &S->P_data[1][0];             mreal const * const restrict N1 = &S->P_data[4][0];
    mreal const * const restrict X2 = &S->P_data[2][0];             mreal const * const restrict N2 = &S->P_data[5][0];
    mreal const * const restrict X3 = &S->P_data[3][0];             mreal const * const restrict N3 = &S->P_data[6][0];

    mreal const * const restrict B  = &T->P_data[0][0];
    mreal const * const restrict Y1 = &T->P_data[1][0];             mreal const * const restrict M1 = &T->P_data[4][0];
    mreal const * const restrict Y2 = &T->P_data[2][0];             mreal const * const restrict M2 = &T->P_data[5][0];
    mreal const * const restrict Y3 = &T->P_data[3][0];             mreal const * const restrict M3 = &T->P_data[6][0];
